...
class ExcelProcessor:
    ...
    def load_vendas(self, file_path: str) -> Dict[str, Any]:
        """Carregar planilha VENDAS/contratos - múltiplas abas"""
        try:
            logger.info(f"📊 Carregando VENDAS: {file_path}")
            ...
            # Processar todas as abas da planilha, independente do nome
            for sheet_name in excel_file.sheet_names:
                # Ignorar abas de controle/dashboard comuns
                ignore_sheets = ['PAINEL', 'DESHBOARD', 'DASH', 'CONTROLE', 'LAYOUT']
                if any(ignore in sheet_name.upper() for ignore in ignore_sheets):
                    logger.info(f"   ⏭️ Ignorando aba de controle: {sheet_name}")
                    continue
                ...
                try:
                    ...
                    if not df_sheet.empty:
                        # Verificar se tem coluna protocolo
                        protocol_col = self._find_protocol_column(df_sheet)
                        
                        if protocol_col:
                            self.df_vendas_sheets[sheet_name] = {
                                'dataframe': df_sheet,
                            ...
                            total_records += len(df_sheet)
                            logger.info(f"   ✅ {sheet_name}: {len(df_sheet)} registros")
                        else:
                            logger.warning(f"   ⚠️ {sheet_name}: Sem coluna protocolo")
                                
                except Exception as e:
                    logger.error(f"   ❌ Erro na aba {sheet_name}: {e}")
            
            if not self.df_vendas_sheets:
                return {
                    "success": False,
                    "error": "Nenhuma aba válida encontrada na planilha VENDAS"
                }
            
            logger.info(f"✅ VENDAS carregada: {len(sheets_found)} abas, {total_records} registros")
            
            return {
                "success": True,
                "total_records": total_records,
                "sheets": sheets_found,
                "sheets_count": len(sheets_found)
            }
    ...
    def process_matching(self) -> Dict[str, Any]:
        """Processar matching entre FPD e VENDAS pelos protocolos"""
        try:
            if self.df_fpd is None or not self.df_vendas_sheets:
                raise Exception("Carregue FPD e VENDAS primeiro")
            
            logger.info("🔄 Processando matching FPD x VENDAS...")
            
            # Obter protocolos da FPD filtrada
            fpd_filtered = self._apply_basic_filters()
            fpd_protocols = set(fpd_filtered[self.protocolo_column].dropna().astype(str))
            
            matched_records = []
            total_protocols_vendas = 0
            
            # Buscar em todas as abas de VENDAS
            for sheet_name, sheet_data in self.df_vendas_sheets.items():
                df_vendas = sheet_data['dataframe']
                vendas_protocol_col = sheet_data['protocol_column']
                
                # Protocolos da aba
                vendas_protocols = df_vendas[vendas_protocol_col].dropna().astype(str)
                total_protocols_vendas += len(vendas_protocols)
                
                # Encontrar matches
                for _, fpd_row in fpd_filtered.iterrows():
                    fpd_protocol = str(fpd_row[self.protocolo_column])
                    
                    if fpd_protocol in vendas_protocols.values:
                        # Encontrar linha correspondente na VENDAS
                        vendas_match = df_vendas[df_vendas[vendas_protocol_col].astype(str) == fpd_protocol]
                        
                        if not vendas_match.empty:
                            vendas_row = vendas_match.iloc[0]
                            
                            # Criar registro combinado
                            combined_record = {
                                'protocolo': fpd_protocol,
                                'fpd_data': fpd_row.to_dict(),
                                'vendas_data': vendas_row.to_dict(),
                                'vendas_sheet': sheet_name,
                                # Campos importantes para cobrança
                                'nome_cliente': self._extract_client_name(vendas_row),
                                'telefone': self._extract_phone(vendas_row),
                                'cpf_cnpj': self._extract_document(vendas_row),
                                'valor_fpd': fpd_row.get('cobrado_fpd', 0),
                                'data_vencimento': fpd_row.get('data_vencimento_fpd'),
                                'status_pagamento': fpd_row.get('faixa_pgto_fpd', ''),
                                'ready_for_cobranca': True
                            }
                            
                            matched_records.append(combined_record)
            
            self.matched_data = matched_records
            
            logger.info(f"✅ Matching concluído: {len(matched_records)} matches de {len(fpd_protocols)} protocolos FPD")
            
            return {
                "success": True,
                "matched": len(matched_records),
                "total_protocols": len(fpd_protocols),
                "total_vendas_protocols": total_protocols_vendas,
                "ready_for_cobranca": len([r for r in matched_records if r['ready_for_cobranca']])
            }
